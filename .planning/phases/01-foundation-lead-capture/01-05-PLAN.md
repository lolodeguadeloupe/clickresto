...
must_haves:
  truths:
    ...
    - "New users automatically get affiliate role via database trigger"
    - "Affiliates cannot access /admin/* routes and see affiliate-appropriate content"
  artifacts:
...
<task type="auto">
  <name>Task 3: Create admin leads page and components with AUTH-04 verification</name>
  <files>
    backoffice/app/(dashboard)/admin/leads/page.tsx
    backoffice/components/dashboard/LeadsTable.tsx
    backoffice/components/dashboard/LeadStatusBadge.tsx
  ...
  <verify>
    ...
    Verify AUTH-04 (role separation) implementation:
    ```bash
    # Verify requireAdmin is called in admin page
    grep -q "requireAdmin" backoffice/app/\(dashboard\)/admin/leads/page.tsx && echo "Admin protection OK" || echo "FAIL: Missing requireAdmin"

    # Verify affiliate role auto-assignment trigger exists in schema
    grep -q "handle_new_user" supabase/schema.sql && echo "Auto-role trigger OK" || echo "FAIL: Missing auto-role trigger"
    grep -q "'affiliate'" supabase/schema.sql && echo "Affiliate role in trigger OK" || echo "FAIL: Trigger doesn't set affiliate role"

    # Verify sidebar shows different content based on role (isAdmin check)
    grep -q "isAdmin" backoffice/components/dashboard/Sidebar.tsx && echo "Role-based sidebar OK" || echo "FAIL: Sidebar not role-aware"
    grep -q "!isAdmin" backoffice/components/dashboard/Sidebar.tsx && echo "Affiliate content check OK" || echo "FAIL: No affiliate-specific content"
    ```
  </verify>
  <done>
    - Admin leads page at /admin/leads
    - requireAdmin() protection - non-admins redirected to /dashboard
    ...
    - AUTH-04 verification: auto-role trigger creates affiliate role for new users
    - AUTH-04 verification: sidebar shows different navigation for admin vs affiliate
  </done>
</task>
...
3. Sidebar shows different links based on user role (admin sees Leads, affiliate sees Affiliation section)
4. Admin leads page requires admin role (requireAdmin call verified)
5. LeadsTable displays leads with all fields
6. LeadStatusBadge shows colored French labels
7. French text throughout
8. AUTH-04 complete: Database trigger assigns 'affiliate' role to new users (verified in schema.sql)
9. AUTH-04 complete: Affiliates cannot access /admin/* (requireAdmin redirects to /dashboard)
10. AUTH-04 complete: Affiliates see affiliate-appropriate dashboard content (!isAdmin section in Sidebar and DashboardPage)
</verification>

<success_criteria>
- Admin can access /admin/leads
- Non-admin redirected to /dashboard (requireAdmin enforced)
- New users automatically get 'affiliate' role via database trigger (handle_new_user)
- Affiliates see affiliate-specific navigation in sidebar (Mes parrainages section)
- Affiliates see affiliate-appropriate content on dashboard page (parrainage placeholder)
