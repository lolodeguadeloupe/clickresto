...
<task type="auto">
  ...
  <verify>
    ...
    Verify key_links wiring exists:
    ```bash
    grep -q "createServerClient" backoffice/middleware.ts && echo "middleware wiring OK" || echo "FAIL: middleware missing createServerClient"
    grep -q "cookies()" backoffice/lib/supabase/server.ts && echo "server.ts wiring OK" || echo "FAIL: server.ts missing cookies()"
    ```
  </verify>
  <done>
    ...
    - middleware.ts exists with session refresh using createServerClient
    - server.ts uses cookies() API
    - Build passes
  </done>
</task>
...
5. Key links verified: middleware.ts uses createServerClient, server.ts uses cookies()
</verification>
